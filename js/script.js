window.onload = function()
{
    ...
    */
    ...
    var decimalFraccionario = function()
    {
        ...
        */
        ...
        var numeroCompleto=Number(datosDecimal.entero+"."+datosDecimal.decimal);
        //console.log(numeroCompleto);
        var elevar = Math.pow(10,datosDecimal.decimal.toString().length);
        var numerador_Fraccionario=numeroCompleto*elevar;
        var fraccionario_Completo=String(numerador_Fraccionario+"/"+elevar)
        nom_div("resFraccionario").innerHTML =   simplificaFraccionario(fraccionario_Completo);
    };
  ...
  var respuestaBinario;
    var binarioDecimal = function()
    {
        /*
            Se deberá validar que los valores ingresados sean 1 ó 0...
      ...
      respuestaBinario=0; //Reiniciar variable
      for(var i = numeroInputs, exponente = 0; i >= 1; i--, exponente++)
        {
          if(nom_div("valor_" + i).value==="1")
            {
              nom_div("resDecimal").innerHTML= respuestaBinario+=Math.pow(2,exponente);
            }
          else
          {
            nom_div("resDecimal").innerHTML= respuestaBinario;
          }
        }
    };
    ...
    nom_div("maximo").addEventListener('change', function(event)
    {
      var res=0;
        ...
        for (var i = 1; i < rangoMultiplos; i++)
        {
          if (i%5===0||i%3===0)
          {
            nom_div("resSumatoria").innerHTML = res+=i;
          }
        }
    });
